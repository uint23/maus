...
static int8_t handle_event(const MSG* msg, MausEvent* ev, Maus* mw);
...
static int8_t handle_event(const MSG* msg, MausEvent* ev, Maus* mw)
{
	...
	switch (msg->message) {
		...
		case WM_LBUTTONDOWN:
			...
			mw->mouse_buttons[MAUS_MOUSE_BUTTON_LEFT] = 1;
			return 1;
		case WM_LBUTTONUP:
			ev->type = MAUS_EV_MOUSE_BUTTON;
			ev->mouse.button.button = MAUS_MOUSE_BUTTON_LEFT;
			ev->mouse.button.pressed = 0;
			mw->mouse_buttons[MAUS_MOUSE_BUTTON_LEFT] = 0;
			return 1;
		...
		case WM_RBUTTONDOWN:
			...
			ev->mouse.button.pressed = 1;
			mw->mouse_buttons[MAUS_MOUSE_BUTTON_RIGHT] = 1;
			return 1;
		case WM_RBUTTONUP:
			ev->type = MAUS_EV_MOUSE_BUTTON;
			ev->mouse.button.button = MAUS_MOUSE_BUTTON_RIGHT;
			ev->mouse.button.pressed = 0;
			mw->mouse_buttons[MAUS_MOUSE_BUTTON_RIGHT] = 0;
			return 1;
		...
		case WM_MBUTTONDOWN:
			...
			ev->mouse.button.pressed = 1;
			mw->mouse_buttons[MAUS_MOUSE_BUTTON_MIDDLE] = 1;
			return 1;
		case WM_MBUTTONUP:
			ev->type = MAUS_EV_MOUSE_BUTTON;
			ev->mouse.button.button = MAUS_MOUSE_BUTTON_MIDDLE;
			ev->mouse.button.pressed = 0;
			mw->mouse_buttons[MAUS_MOUSE_BUTTON_MIDDLE] = 0;
			return 1;
...
static char translate_text(UINT vk, UINT scan)
{
	...
	n = ToUnicode(vk, scan, state, buf, 4, 0);
...
static LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
{
	...
	if (msg == WM_SIZE) {
		mw = (Maus*)GetWindowLongPtr(hwnd, GWLP_USERDATA);
		if (mw) {
			be = &mw->backend;
			be->resized = 1;
			be->rw = LOWORD(lp);
